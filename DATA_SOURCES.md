...
| **LLM Explanation & Assistant** | Scheme Match Natural Rephrasing & Follow-up Assistant | **AI EXPLANATION LAYER (REPHRASING ONLY)** | Groq API (`https://api.groq.com/openai/v1`, Llama-family model) | LLM explanation & assistant layer — Groq API (Llama-family model), rephrases deterministic rules-engine output only; does not independently determine eligibility facts. Validated against source figures before display; falls back to plain text on mismatch or API failure. |

---

## 2.1 LLM Explanation & Follow-Up Assistant Layer

SahayaSetu introduces an LLM intelligence layer on top of the deterministic rules engine — **without replacing it**:

1. **Deterministic Rule Engine as Single Source of Truth**:
   The rules engine remains the sole authority for evaluating eligibility, loan caps, margin money requirements, interest rates, and repayment tenures based on official NSFDC statutory guidelines.
2. **Groq API Integration (Llama-family model)**:
   The backend routes `/api/explain` and `/api/assistant/chat` make server-to-server calls to Groq's OpenAI-compatible API (`https://api.groq.com/openai/v1`) using Meta Llama models (e.g. `llama-3.3-70b-versatile` / `llama-3.1-8b-instant`).
3. **Automated Fact Validation Guardrail**:
   Every response generated by `/api/explain` is scanned by an automated fact extractor. The validator verifies that all monetary figures (₹), percentages (%), and moratorium durations mentioned in the output strictly match the input facts. If any unvetted or hallucinated figure is detected (e.g., an unauthorized discount or fee), the LLM text is instantly discarded and the system falls back to the deterministic rules engine string.
4. **Resilience & Graceful Offline Fallbacks**:
   If the Groq API key is not configured, or if the external API experiences rate limits (429), timeouts (>8s), or network downtime, the system falls back seamlessly to the deterministic rules reasoning for Feature 1, and shows a friendly temporary unavailability message for Feature 2. The core Recommender, Financial Math, and Partner Locator flows continue to operate with 100% availability.
5. **Zero Client Exposure of Credentials**:
   The `GROQ_API_KEY` is maintained exclusively server-side in `.env`, is strictly excluded from version control via `.gitignore`, and is never exposed in client bundles or public repositories.
